(function( $, icons ){
	...
	var add_icon = function( $input, icon_url ){
		if ( "msi-custom-menu-item-url" == $input.attr('id') ) {
	...
	var display_icon_shortcuts = function(){
		...
		$('#asi-social-icons-custom').append( $wrapper );
	};
	...
	var update_link_fields = function(){
		...
		$('#msi-custom-menu-item-url').val( 'http://' + url );
		$('#msi-custom-menu-item-name').val( name ).removeClass('input-with-default-title');
	};
	...
	var init = function(){
		...
		$( '#msi-custom-menu-item-url' ).on( 'keyup', check_menu_item );

		// Closest thing I can find to an event on new link add
		$( '#msi-custom-menu-item-name' ).on( 'blur', function(){
			$( '#menu-to-edit input.edit-menu-item-url' ).each( check_menu_item );
			remove_icon_from_new_link();
		});
...
jQuery(document).ready(function(){
	jQuery(document).on('change','.slect-menu-item-size', function(){
		getidval = jQuery(this).attr('id');
		var selectedSize = jQuery(this).children("option:selected").val();
		if(selectedSize == 'other')
		{
			jQuery('#menu-item-iconsize-'+getidval).css('display','block');
		}
		else{
			jQuery('#menu-item-iconsize-'+getidval).css('display','none');
		}
	});
});	
